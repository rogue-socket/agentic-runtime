...
def resolve_function(ref: str, functions_dir: str) -> Callable:
    ...
    Raises:
        FunctionResolutionError: If the function cannot be found or is ambiguous.
    """
...
def _resolve_qualified(ref: str, functions_dir: str) -> Callable:
    ...
    parts = ref.rsplit(".", 1)
    if len(parts) != 2:
        raise FunctionResolutionError(f"Invalid function reference: {ref}")
    ...
    if not os.path.isfile(module_file):
        raise FunctionResolutionError(
            ...
            f"Create {module_file} with a top-level callable named '{func_name}', "
            f"or fix the reference in your workflow YAML."
        )
...
def _import_from_path(filepath: str, module_name: str):
    ...
    if spec is None or spec.loader is None:
        raise FunctionResolutionError(f"Cannot import module from {filepath}")
